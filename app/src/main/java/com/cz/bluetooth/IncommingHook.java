...
public class IncommingHook {
    ...
    private static final String INCOMING_WINDOW_METHOD = "l";
     ...
     * Hook 来电悬浮窗大小
     */
    public void hookIncomingWindowSize(String tag, ClassLoader cl) {
        XposedBridge.log("[BluetoothHook][" + tag + "] start hook incoming window size in " + cl);
        try {
            // 找到 K0.h 类 (来电悬浮框)
            XposedBridge.log("[BluetoothHook][" + tag + "] trying to find " + INCOMING_WINDOW_CLASS + " class");
            Class<?> incomingViewClass = XposedHelpers.findClass(INCOMING_WINDOW_CLASS, cl);
            XposedBridge.log("[BluetoothHook][" + tag + "] found " + INCOMING_WINDOW_CLASS + " class: " + incomingViewClass);

            // Hook l() 方法，调整窗口大小
            XposedBridge.log("[BluetoothHook][" + tag + "] hooking " + INCOMING_WINDOW_METHOD + "() method");
            Class<?> c0119bClass = XposedHelpers.findClass("c0.b", cl);
            XposedHelpers.findAndHookMethod(incomingViewClass, INCOMING_WINDOW_METHOD, android.content.Context.class, c0119bClass, new XC_MethodHook() {
                @Override
                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                    XposedBridge.log("[BluetoothHook][" + tag + "] after " + INCOMING_WINDOW_METHOD + "() method");
                    super.afterHookedMethod(param);

                    try {
                        // 获取 K0.h 实例
                        Object incomingViewInstance = param.thisObject;
                        XposedBridge.log("[BluetoothHook][" + tag + "] got incomingViewInstance: " + incomingViewInstance);
                        ...
                        Object context = null;
                        Object view = null;
                        Class<?> cls = incomingViewInstance.getClass();
                        ...
                        for (java.lang.reflect.Field field : fields) {
                            ...
                            try {
                                Object value = field.get(incomingViewInstance);
                                ...
                                else if (value instanceof View && !(value instanceof ImageView) && !(value instanceof TextView)) {
                                    view = value;
                        ...
                        if (params != null) {
                            XposedBridge.log("[BluetoothHook][" + tag + "] original incoming window size: " + params.width + "x" + params.height);

                            // 修改窗口大小为原始大小的一半
                            params.width = params.width > 0 ? params.width / 2 : params.width;
                            params.height = params.height > 0 ? params.height / 2 : params.height;
                            XposedBridge.log("[BluetoothHook][" + tag + "] new incoming window size: " + params.width + "x" + params.height);

                            // 读取缓存的位置
                            int savedX = -1;
                            if (sharedPreferences != null) {
                                try {
                                    savedX = (Integer) XposedHelpers.callMethod(sharedPreferences, "getInt", "IncomingViewX", -1);
                        ...
                        // 兜底，防止崩溃
                        XposedBridge.log("[BluetoothHook][" + tag + "] failed to adjust incoming window size");
                        XposedBridge.log(t);
                    }
                }
            });
            ...
            XposedBridge.log(
                    "[BluetoothHook][" + tag + "] hook incoming window size success in " + cl
            );

        } catch (Throwable t) {
            // 类不存在 / ClassLoader 不对 / 尚未加载
            XposedBridge.log(
                    "[BluetoothHook][" + tag + "] hook incoming window size failed"
            );
            XposedBridge.log(t);
        }
    }
                    ...
                    if (!viewSizeCache.containsKey(viewKey)) {
                        ...
                        try {
                            ...
                            if (layoutParams != null) {
                                ...
                                XposedBridge.log("[BluetoothHook] cached view size info for " + viewKey);
                            }
                        } catch (Throwable t) {
